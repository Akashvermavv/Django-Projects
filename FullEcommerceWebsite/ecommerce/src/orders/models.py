...
class OrderManagerQuerySet(models.query.QuerySet):
    ...
    def by_weeks_range(self,week_ago=7,number_of_weeks=2):
        if number_of_weeks > week_ago:
            number_of_weeks = week_ago
        print('number of weeks in models --->',number_of_weeks)
        print('number of weeks ago in models --->',number_of_weeks)
        days_ago_start = week_ago * 7   # days_ago_start = 49
        days_ago_end   =days_ago_start - (number_of_weeks*7)  # days_ago_end = 49-14=35
        print('days ago end in models -->',days_ago_end)
        start_date = timezone.now()  - datetime.timedelta(days=days_ago_start)
        print('start_date in models ->',start_date)
        end_date = timezone.now() - datetime.timedelta(days=days_ago_end)
        print('end_date in models ->', end_date)
    ...
    def by_date(self):
        # return self.filter(updated__year=datetime.datetime.now().year)
        # return self.filter(updated__year=datetime.datetime.now().year)
...
class Order(models.Model):
    ...
    order_id = models.CharField(max_length=120,blank=True,null=True)  # e.g -- AB31DE3
    ...
    def update_total(self):
        print('in update_total --')
        cart_total = self.cart.total
        shipping_total = self.shipping_total
        # print('cart total --',cart_total)
        # print('shipping total --',shipping_total)

        new_total = fsum([cart_total , shipping_total])
        formatted_total = format(new_total,'.2f')

        # print('formatted_total is--',formatted_total)
...
def post_save_cart_total(sender,instance,created,*args,**kwargs):
    if not created:
        ...
        if qs.count()==1:
            print('updating already exist order')
            order_obj = qs.first()
